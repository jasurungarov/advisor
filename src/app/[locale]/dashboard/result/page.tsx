      ...
      {assessments.length === 0 ? (
        <Card className="border-0 shadow-lg dark:bg-[#0e4a38]">
          <CardContent className="p-12 text-center">
            ...
            <p className="text-[#4a6e5d] mb-6">{t("history.empty.subtitle")}</p>
        ...
        <div className="space-y-4">
          {assessments.map((assessment) => (
            <Card
              key={assessment._id}
              className="border-0 shadow-lg hover:shadow-xl transition-shadow dark:bg-[#0e4a38]">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <h3 className="text-lg font-semibold text-brand-900 dark:text-brand-100 mb-2">
                      {t("history.topMatch")}:{" "}
                      {assessment.recommendations[0]?.specialization?.name}
                    </h3>
                      ...
                      {assessment.recommendations
                        ...
                        .map((rec, index) => (
                          <Badge
                            ...
                            #{index + 1} {rec.specialization.name} (
                            {rec.matchScore}%)
                          </Badge>
                        ))}
                    </div>
                  ...
                  <div className="flex flex-col gap-2 md:items-end">
                    <Link
                      href={`/dashboard/result/${assessment._id}`}
                      prefetch={false}
                      className="w-full block" // ← block va w-full
                    >
                      <Button
                        variant="outline"
                        size="lg"
                        className="w-full gap-2 rounded-full dark:bg-primary/20 bg-brand-500 text-brand-900 font-semibold hover:bg-brand-700 shadow-brand-500/30 border border-primary transition-shadow duration-300 hover:shadow-[0_0_30px_rgba(0,0,0,0.18)] dark:shadow-primary dark:hover:shadow-primary shadow">
